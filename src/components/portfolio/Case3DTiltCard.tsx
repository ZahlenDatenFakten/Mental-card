...
export const Case3DTiltCard: React.FC<Case3DTiltCardProps> = ({
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setRotateX(-normY * 9); // tilt on X axis
    setRotateY(normX * 9); // tilt on Y axis
    ...
    switch (inst) {
      case 'district':
        return {
          ...
          short: 'Окружной суд',
          icon: <Building2 className="w-4 h-4 text-[#0A84FF]" />,
      ...
      case 'appellate':
        return {
          ...
          icon: <Scale className="w-4 h-4 text-[#BF5AF2]" />,
      ...
      case 'supreme':
        return {
          ...
          short: 'Верховный Суд',
          icon: <Crown className="w-4 h-4 text-[#30D158]" />,
  ...
  // Extract up to 4 preview branch names
  ...
  return (
    <div
      ...
      style={{
        transform: `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateZ(${rotateX !== 0 ? 8 : 0}px)`,
        transition: rotateX === 0 ? 'transform 400ms cubic-bezier(0.25, 1, 0.5, 1)' : 'none',
      }}
      className={`relative p-6 rounded-3xl border select-none transition-all duration-150 flex flex-col justify-between group cursor-pointer overflow-hidden ${
        isActive
          ? 'bg-zinc-900/90 border-[#0A84FF] ring-2 ring-[#0A84FF]/40 shadow-[0_16px_48px_rgba(10,132,255,0.25)]'
          : 'bg-[#18181b]/80 hover:bg-[#202024]/90 border-white/[0.12] hover:border-white/[0.24] shadow-[0_12px_36px_rgba(0,0,0,0.6)]'
      }`}
    >
      {/* Dynamic Ambient Instance Aura Glow */}
      <div
        className="absolute -top-16 -right-16 w-44 h-44 rounded-full pointer-events-none blur-3xl opacity-40 group-hover:opacity-75 transition-opacity"
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="p-3.5 bg-black/40 border border-white/[0.08] rounded-2xl space-y-2">
          <div className="flex items-center justify-between text-[11px] text-[#8E8E93]">
            <span className="flex items-center gap-1.5 font-medium text-[#EBEBF5]">
              ...
              <span>Структура дела ({totalNodes} блоков)</span>
            </span>
          ...
          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
            {previewBranches.map((child, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 text-[10.5px] px-2 py-0.5 rounded-md bg-white/[0.06] border border-white/[0.1] text-[#EBEBF5] truncate max-w-[150px]"
              >
                <span
      ...
      {/* Bottom Actions Bar */}
      <div className="relative z-10 mt-5 pt-4 border-t border-white/[0.08] flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {caseItem.instance === 'district' && (
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium text-[#E0B0FF] hover:text-white bg-[#BF5AF2]/20 hover:bg-[#BF5AF2]/35 px-3 py-2 rounded-xl border border-[#BF5AF2]/40 transition-all active:scale-[0.95] cursor-pointer"
            >
          ...
          {caseItem.instance === 'appellate' && (
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium text-[#30D158] hover:text-white bg-[#30D158]/20 hover:bg-[#30D158]/35 px-3 py-2 rounded-xl border border-[#30D158]/40 transition-all active:scale-[0.95] cursor-pointer"
            >
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-2 text-[#8E8E93] hover:text-white hover:bg-white/[0.1] rounded-xl transition-all active:scale-[0.9] cursor-pointer"
          >
